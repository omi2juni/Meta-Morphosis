...
function setup() {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      input.position(cnvx + x[7], cnvy + y[3] - 10);
      input2.position(cnvx + x[7], cnvy + y[7] - 10);
      input3.position(cnvx + x[7], cnvy + y[11] - 10);

      checkbox1.position(cnvx + x[16], cnvy + y[3]);
      checkbox2.position(cnvx + x[16], cnvy + y[7]);
      checkbox3.position(cnvx + x[16], cnvy + y[11]);

      button.position(cnvx + x[7], cnvy + (y[15] + 11), "absolute");
    }
  }
}
